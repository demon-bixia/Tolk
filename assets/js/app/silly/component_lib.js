...
let ComponentsModels = {
    ...
    'conversations-list': {
        ...
        render_mode: 'replaceOrAppend',
        template: `<% if (conversations.length===0){%> <p class="empty-section">to start a conversation press the <i class="flaticon-add-button"></i> button</p><%}else{%><ul class="nav discussion" role="tablist"> <% for(let conversation of conversations){%> <% if (conversation.type==='couple'){%> <% for(let participant of conversation.participants){%> <% if (participant.id !== authenticated_contact.id){%> <li> <a href="#conversation_<%=conversation.id %>" class="filter direct" data-id="<%= conversation.id %>" data-name="<%=conversation.name %>" data-user="<%=participant.user %>" data-toggle="tab" data-chat="open" role="tab" aria-controls="<%=conversation.data %>" aria-selected="true"> <div class="status online"> <div class="circle-status"> <img src="<%=participant.contact_pic %>" alt="avatar"> </div></div><div class="content"> <div class="headline"><h5><%=participant.first_name %> <%=participant.last_name %></h5> <span> <% if (conversation.last_message_date){%> <%=conversation.last_message_data %> <%}%> </span> </div><p><% if (conversation.last_message){%> <%=conversation.last_message.slice(0, 20) %>... <%}%></p></div></a> </li><%}%> <%}%> <%}%> <% if (conversation.type==='group'){%> <li> <a href="#conversation_<%=conversation.id %>" class="filter direct" data-id="<%= conversation.id %>" data-name="<%=conversation.name %>" data-toggle="tab" data-chat="open" role="tab" aria-controls="<%=conversation.name %>" aria-selected="true"> <div class="status online"> <div class="circle-status"> <img src="<%=conversation.header %>" alt="avatar"> </div></div><div class="content"> <div class="headline"><h5><%=conversation.name %></h5> <span> <% if (conversation.last_message_date){%> <%=conversation.last_message_data %> <%}%> </span> </div><p><% if (conversation.last_message){%> <%=conversation.last_message.slice(0, 20) %>... <%}%></p></div></a> </li><%}%> <%}%></ul><%}%>`,
    },

    'conversation-bubble': {
        render_mode: 'append',
        template: `<% if (conversation.type==='couple'){%> <% for(let participant of conversation.participants){%> <% if (participant.id !== authenticated_contact.id){%> <li> <a href="#conversation_<%=conversation.id %>" class="filter direct" data-id="<%= conversation.id %>" data-name="<%=conversation.name %>" data-user="<%=participant.user %>" data-toggle="tab" data-chat="open" role="tab" aria-controls="<%=conversation.data %>" aria-selected="true"> <div class="status online"> <div class="circle-status"> <img src="<%=participant.contact_pic %>" alt="avatar"> </div></div><div class="content"> <div class="headline"><h5><%=participant.first_name %> <%=participant.last_name %></h5> <span> <% if (conversation.last_message_date){%> <%=conversation.last_message_data %> <%}%> </span> </div><p><% if (conversation.last_message){%> <%=conversation.last_message.slice(0, 20) %>... <%}%></p></div></a> </li><%}%> <%}%> <%}%> <% if (conversation.type==='group'){%> <li> <a href="#conversation_<%=conversation.id %>" class="filter direct" data-id="<%= conversation.id %>" data-name="<%=conversation.name %>" data-toggle="tab" data-chat="open" role="tab" aria-controls="<%=conversation.name %>" aria-selected="true"> <div class="status online"> <div class="circle-status"> <img src="<%=conversation.header %>" alt="avatar"> </div></div><div class="content"> <div class="headline"><h5><%=conversation.name %></h5> <span> <% if (conversation.last_message_date){%> <%=conversation.last_message_data %> <%}%> </span> </div><p><% if (conversation.last_message){%> <%=conversation.last_message.slice(0, 20) %>... <%}%></p></div></a> </li><% } %>`
    },

    'bubble-last-message': {
        render_mode: 'replaceOrAppend',
        template: `<p><% if (!message.is_file) { %>  <%= message.content.slice(0, 40) %>  <% } else { %> <%= message.attachments[0].file_name.slice(0, 20) %> <% } %>...</p>`,
    },
    ...
    'first-notification-bubble': {
        ...
        render_mode: 'replaceOrPrepend',
        template: `<ul class="notifications" role="tablist"> <% if (notification.type==='authentication'){%><li><div class='round'><i data-eva="lock"></i></div><p><strong><%=notification.type %></strong> <%=notification.content %></p></li><%}else if (notification.type==='accounts'){%><li><div class='round'><i data-eva="people"></i></div><p><strong><%=notification.type %></strong> <%=notification.content %></p></li><%}else if (notification.type==='chat'){%><li><div class='round'><i data-eva="message-circle"></i></div><p><strong><%=notification.type %></strong> <%=notification.content %></p></li><%}else if (notification.type==='default'){%><li><div class='round'><i data-eva="checkmark-circle"></i></div><p><strong><%=notification.type %></strong> <%=notification.content %></p></li><%}%></ul>`,
    },
    ...
    'message': {
        render_mode: 'append',
        template: `<li class="<% if (message.sender.user == authenticated_user) { %>  self  <% } else { %>  odd  <% } %> "><img src="<%=message.sender.contact_pic %>" alt="avatar"> <div class="content"> <% if (message.is_file){%> <div class="message downloadable" data-url="<%= message.attachments[0].file %>"> <div class="bubble"> <p> <span><i data-eva="arrow-downward-outline"></i></span> <%= message.attachments[0].file_name.slice(0, 10) %> </p></div></div><span> <%=message.time_sent%> </span> <% } else { %> <div class="message"> <div class="bubble"> <p> <%= message.content %> </p></div></div><span> <%= message.time_sent %> </span> <% } %></div></li>`
    },
    ...
    'conversation-create-form': {
        ...
        selector: '#create',
        render_mode: 'replaceOrAppend',
        template: `<div class="modal fade" id="create" tabindex="-1" role="dialog" aria-label="create" aria-hidden="true"> <div class="modal-dialog modal-dialog-centered" role="document"> <div class="modal-content"> <div class="modal-header"> <h5>Compose</h5> <button type="button" class="button round" data-dismiss="modal" aria-label="close"><i data-eva="close"></i></button> </div><div class="modal-body"> <ul class="nav" role="tablist"> <li><a href="#contact" class="active" data-toggle="tab" aria-controls="details" aria-selected="true">contact </a></li><li><a href="#couple" class="" data-toggle="tab" aria-controls="details" aria-selected="false">conversation </a> </li><li><a href="#group" data-toggle="tab" role="tab" aria-controls="participants" aria-selected="false"> Group </a></li></ul> <div class="tab-content"> <div class="details tab-pane fade show active" id="contact" role="tabpanel"> <form id="contact-form" data-contact="<%=authenticated_contact.id %>"> <div class="form-group"> <label>Email</label> <input type="email" placeholder="Email Address" name="email" class="form-control"> <div class="error_container"></div></div><div class="form-group"> <button type="submit" class="button btn-primary">Compose</button> </div></form> </div><div class="participants tab-pane fade" id="group" role="tabpanel"> <form action="" id="group-create" data-contact="<%=authenticated_contact.id %>" enctype="multipart/form-data"> <div class="form-group"> <label for="">Name</label> <input type="text" placeholder="Name" name="group_name" class="form-control"> <div class="error_container"></div></div><div class="form-group"> <label for="">Header</label> <div class="custom-file"> <input type="file" class="custom-file-input" id="customFile" name="group_header"> <label class="custom-file-label" for="customFile">Choose Header file</label> </div><div class="error_container"></div></div><h4>Contacts</h4> <hr> <ul class="users"> <% for (let contact of contacts){%> <% if (contact.id !==authenticated_contact.id && contact.private_mode !==true){%> <li> <div class="status"><img src="<%=contact.contact_pic %>" alt="avatar"></div><div class="content"> <h5><%=contact.first_name[0].toUpperCase() %> <%=contact.last_name[0].toUpperCase() %></h5> <span><%=contact.location %></span></div><div class="custom-control custom-checkbox"> <input type="checkbox" class="custom-control-input" name="contact_<%=contact.id%>" id="contact_<%=contact.id %>" data-contact="<%=contact.id%>"> <label class="custom-control-label" for="contact_<%=contact.id %>"></label> </div></li><%}%> <%}%> </ul> <div class="form-group"> <button type="submit" class="button btn-primary">Compose</button> </div></form> </div><div class="conversation tab-pane fade" id="couple" role="tabpanel"> <div class="contacts list-group scrollbar"> <% for (let contact of contacts){%> <% if (contact.id !==authenticated_contact.id && contact.private_mode !==true){%> <a class="contact list-group-item list-group-item-action" data-authenticated-contact="<%= authenticated_contact.id %>" data-contact="<%= contact.id %>" data-user="<%= contact.user %>"> <div class="status"><img src="<%=contact.contact_pic %>" alt="avatar"></div><div class="content"> <h5><%=contact.first_name[0].toUpperCase() %> <%=contact.last_name[0].toUpperCase() %></h5> <span><%=contact.location %></span> </div></a> <%}%> <%}%> </div><div class="contacts-footer"> <a href="#contact">ADD CONTACT</a> <a href="#" data-dismiss="modal" aria-label="close">CLOSE</a> </div></div></div></div></div></div></div>`,
    },
    ...
    'save-notifications-switch': {
        ...
        render_mode: 'replaceOrAppend',
        template: ` <li class="save-notifications-mode"> <div class="headline"> <h5>Save Notifications</h5> <label class="switch"> <input type="checkbox" name="save_notifications" <% if (settings.save_notifications) { %>checked<% } else { %><% } %> > <span class="slider round"></span> </label> </div><p>don't delete notifications after they are viewed.</p></li>`,
    },
